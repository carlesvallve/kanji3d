...
var getMetrics = function(text, font) {
...
function setShadowStyle(ctx, shadowStyle, text, font, color, offset) {
    ...
    // absolute position of the text (within a translation state)
    var offsetX = offset.x - width / 2;
    var offsetY = offset.y - metrics.height * 0.5; //62;


    var style = shadowStyles[shadowStyle];
    // add a background to the current effect
    //ctx.fillStyle = style.background;
    //ctx.fillRect(0, offsetY, ctx.canvas.width, textHeight);
    // parse text-shadows from css
    var shadows = parseShadow(style.shadow);
    // loop through the shadow collection
    var n = shadows.length; while(n--) {
