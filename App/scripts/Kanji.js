var Kanji = function () {
    ...
    this.generateKanjiTexture = function (text, options, drawingFunction) {
        options = { width: 300, height: 300, fontSize: 200, color: utils.randomArr(['#000000']) }; // 400
    ...
    this.drawPixasticGlowingTexture = function(ctx, text, options) {
        ...
        ctx.fillStyle = '#ffffff'; //'#ff00de'; //'#' + Math.floor(Math.random() * 16777215).toString(16); //'#ffffff'; //options.color;
        ctx.fillText(text, options.width / 2, options.height / 2); // text

        Pixastic.process(ctx.canvas, 'glow', { amount: 1.0, radius: 5.0 }, function (pixCanvas2) {
            pixCanvas2.tagName = 'canvas';
            //Pixastic.process(pixCanvas, 'glow', { amount: 1.0, radius: 1.0 }, function (pixCanvas2) {
                //pixCanvas2.tagName = 'canvas';
